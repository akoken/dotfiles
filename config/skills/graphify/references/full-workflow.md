# Full Graphify Workflow

## Contents

- Usage and repository cloning
- Detection, transcription, structural and semantic extraction
- Graph build, clustering, labels, exports, and cost tracking
- Incremental update and cluster-only branches
- Query, path, explain, add, watch, hooks, and native Codex integration
...
**Fast path — existing graph:** First resolve the user-named target directory and run the workflow from that directory. Then check `graphify-out/graph.json` there. If it exists AND the user's request is a natural-language question about the codebase (e.g. "How does X work?", "What calls Y?", "Trace the data flow through Z") and NOT an explicit rebuild command (`--update`, `--cluster-only`, or a bare path/URL that implies fresh extraction): **skip Steps 1–5 entirely and jump straight to `## For /graphify query`.** Run `graphify query "<question>"` immediately. Do not run detect. Do not check corpus size. Do not ask the user to narrow. The graph is already built — use it.

If no path was given, use the narrowest scope explicitly named by the user; otherwise use `.`. Do not expand a named scope to the whole repository.

If the path argument starts with `https://github.com/` or `http://github.com/`, treat it as a GitHub URL - run Step 0 before anything else, then continue with the resolved local path.

Follow these steps in order. Do not skip steps.

**Execution-root invariant:** For a single local directory or cloned repository, resolve its absolute path, change into that directory before Step 1, and use `INPUT_PATH=.` throughout the pipeline. All relative `graphify-out/` paths below therefore belong to the scanned target. Do not run the pipeline from a parent repository while passing a child path. The multi-repository and multi-subfolder CLI workflows below are the only exceptions.
...
find graphify-out -maxdepth 1 -type f -name '.graphify_chunk_*.json' -delete
