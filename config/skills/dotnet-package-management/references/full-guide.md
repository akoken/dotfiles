...
Use repository tooling to establish CPM; the resulting `Directory.Packages.props` in the solution root has this shape (not a manual XML editing instruction):
...
```bash
# With CPM: use the CLI and inspect the central version diff
...
For solution-specific sources, use `dotnet nuget` or repository tooling; the resulting `NuGet.config` has this shape:
