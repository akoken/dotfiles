...
```bash
"${CODEX_HOME:-$HOME/.codex}/skills/agent-guard/bin/agent-guard" check
```

Codex reads user skills from `~/.codex/skills` (`CODEX_HOME` overrides it); all four harness skill dirs resolve to the shared `config/skills/agent-guard`.
...
```bash
"${CODEX_HOME:-$HOME/.codex}/skills/agent-guard/bin/agent-guard" scan-staged
```

3. Use a working-tree scan after agent edits or before a PR:

```bash
"${CODEX_HOME:-$HOME/.codex}/skills/agent-guard/bin/agent-guard" scan-working-tree
```

4. Use path scans for explicit directories or files:

```bash
"${CODEX_HOME:-$HOME/.codex}/skills/agent-guard/bin/agent-guard" scan-path .
```

5. If `gitleaks` is unavailable and the scan cannot run, use the lightweight fallback without printing secret values:

```bash
python3 "${CODEX_HOME:-$HOME/.codex}/skills/agent-guard/scripts/guard.py" --format md
python3 "${CODEX_HOME:-$HOME/.codex}/skills/agent-guard/scripts/guard.py" --files path/to/file --format md
```
...
```bash
"${CODEX_HOME:-$HOME/.codex}/skills/agent-guard/bin/agent-guard" exec -- printenv
```

7. For text that may contain PII:

```bash
printf '%s\n' "$TEXT" | "${CODEX_HOME:-$HOME/.codex}/skills/agent-guard/bin/agent-guard" pii-filter
```

8. To install the stable native git pre-commit hook in the current repo, use the vendored installer from inside that repo:

```bash
"${CODEX_HOME:-$HOME/.codex}/skills/agent-guard/install.sh" git-hooks
```
